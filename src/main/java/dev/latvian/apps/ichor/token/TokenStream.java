...
public class TokenStream {
	...
	private final Stack<Symbol> depth;
	private Symbol currentDepth;
	...
	private Object readToken() {
		if (currentDepth == Symbol.TEMPLATE_LITERAL && peek(1) != '`' && !(peek(1) == '$' && peek(2) == '{')) {
		...
		var s = Symbol.read(this, t);

		if (s == Symbol.COMMENT_LINE) {
			while (true) {
				t = read();
		...
		} else if (s == Symbol.COMMENT_BLOCK_START) {
			while (true) {
				t = read();
		...
		} else if (s == Symbol.DIV && (currentToken == null || currentToken.token instanceof Token tk && tk.isLiteralPre())) {
		...
		} else if (s == Symbol.DOT && isDigit(peek(1))) {
			return readNumber(t); // fix this
		} else if (s == Symbol.SSTRING || s == Symbol.DSTRING) {
			var sb = new StringBuilder();
			...
			if (s == Symbol.TEMPLATE_LITERAL_VAR || s == Symbol.LC || s == Symbol.LP || s == Symbol.LS) {
				depth.push(s);
				currentDepth = s;
			} else if (s == Symbol.RC || s == Symbol.RP || s == Symbol.RS) {
				if (currentDepth == null || depth.isEmpty()) {
					throw error("Unexpected closing bracket " + s);
				}

				test(Symbol.TEMPLATE_LITERAL_VAR, s, Symbol.RC);
				test(Symbol.LC, s, Symbol.RC);
				test(Symbol.LP, s, Symbol.RP);
				test(Symbol.LS, s, Symbol.RS);

				depth.pop();
				currentDepth = depth.isEmpty() ? null : depth.peek();
			} else if (s == Symbol.TEMPLATE_LITERAL) {
				if (currentDepth == Symbol.TEMPLATE_LITERAL) {
					depth.pop();
					currentDepth = depth.isEmpty() ? null : depth.peek();
				} else {
					depth.push(s);
	...
	private void test(Symbol ifToken, Symbol symbol, Symbol expected) {
